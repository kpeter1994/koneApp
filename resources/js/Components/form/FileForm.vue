...
const isFileSelected = ref(false); // Új ref a fájl állapotának nyomon követésére

const handleFileUpload = (event) => {
    form.excel_file = event.target.files[0];
    isFileSelected.value = !!form.excel_file
};
...
<template>
    <form @submit.prevent="submit" class="flex justify-center items-center gap-1.5">
        ...
        <button :disabled="!isFileSelected" :class="{'opacity-25' : !isFileSelected}" class="flex justify-center items-center gap-1.5 px-2 py-1 bg-blue-100 rounded font-semibold" type="submit"><i class="fa-solid fa-file-excel"></i> Importálás</button>
    </form>
